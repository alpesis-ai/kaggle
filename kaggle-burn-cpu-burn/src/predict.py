...
def chooseAlgorithms(algo, train, features, target, test):
    ...
    if algo == 0:
        ...
        clf = createGradientBoosting()
        classify(clf, train, features, target, test, outPath+"submission-gradientboosting.csv")

    elif algo == 8:
        clf = createNaiveBayes()
        classify(clf, train, features, target, test, outPath+"submission-multinorminalNB.csv")
